"""
Claude API fallback for HTML → Elementor JSON conversion.
Used when the deterministic parser produces empty output or the caller
explicitly requests AI-assisted conversion.
"""
...
async def convert_with_claude(raw_html: str) -> list:
    """
    Call the Anthropic Messages API and return parsed Elementor JSON.
    Raises RuntimeError if the API key is missing or the response is invalid.
    """
    ...
    if not api_key or api_key.startswith("sk-ant-xxx"):
        raise RuntimeError(
            "ANTHROPIC_API_KEY is not set. Add it to backend/.env to enable Claude fallback."
        )
    ...
    payload = {
        ...
        "messages": [
            {
                "role": "user",
                "content": f"Convert this HTML to Elementor JSON:\n\n{raw_html}",
            }
        ],
    }
    ...
    if response.status_code != 200:
        raise RuntimeError(
            f"Claude API error {response.status_code}: {response.text[:400]}"
        )
...
# re import needed for fence stripping above
import re  # noqa: E402
