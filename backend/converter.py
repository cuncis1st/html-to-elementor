...
def parse_inline_style(style_str: str) -> dict:
    ...
    if CSSUTILS_AVAILABLE:
        ...
        for prop in sheet:
            ...
            if p == "font-size":
                out["typography_font_size"] = css_size(v)
            if p == "font-weight":
                out["typography_font_weight"] = v
            if p == "text-align":
                out["align"] = v
            if p == "padding":
                out.update(css_shorthand_to_elementor("padding", v))
            if p == "margin":
                out.update(css_shorthand_to_elementor("margin", v))
    else:
        # Minimal fallback without cssutils
...
def youtube_url(tag) -> str | None:
    ...
    m = re.search(
        r"(youtube\.com|youtu\.be)/(?:embed/|watch\?v=)?([A-Za-z0-9_-]{11})", src
    )
...
def html_to_elementor(raw_html: str) -> list:
    """
    Parse raw HTML and return a list of Elementor section dicts.
    This is the deterministic path (no LLM).
    """
    soup = BeautifulSoup(raw_html, "lxml")

    # Strip noise tags (scripts/styles handled by complexity classifier below)
    ...
    for child in body.children:
        ...
        # Whole section is complex → html snippet
        if is_complex(child):
            widget = html_snippet_widget(child)
            ...
            if col_tags:
                ...
                for col_tag in col_tags:
                    ...
                    for c in col_tag.children:
                        if not isinstance(c, Tag) or not c.name:
                            continue
                        w = html_snippet_widget(c) if is_complex(c) else node_to_widget(c)
                        if w:
                            widgets.append(w)
                    if widgets:
